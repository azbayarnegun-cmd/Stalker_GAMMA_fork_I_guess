...
| [Composure suite: Core, Combat, Recovery, Exploration, Consumables, Thoughts, Weapon Handling + Time-Skip, Unified Simulated Time](Composure/NOTES.md) | own mod | Core 2.1.0 (API 2) | yes | Uploaded incl. Feedback 1.2.6. No GAMMA conflicts. Fixed Feedback breathing sound counts |
| [Inventory set: Squared Away, Layered Pouches, HD Icons, FIUT, Seamless Sort + own SA addons 10–95](SquaredAway%20+%20Addon%20Pouch%20+%20Immersive%20Inventory/NOTES.md) | ? / own | SA 3.1.0 / pouches 1.5.3 / addons 10–95 | yes | All complete (Smart Loot Routing disabled, not needed). Overrides GAMMA scripts: HD Icons (8), magazine_binder, QAW MCM |
| [Toxic Air v2 Redux (Priler) + economy rebalance + BigCanisters v2/v3](Toxic%20Air/NOTES.md) | ? / own | Redux / d2026.9.6 / v2 TEST / **v3** | yes | Use **BigCanisters v3** (big tanks + rebalance prices) instead of rebalance and v2. Redux's `trader_autoinject.script` is older than GAMMA's |
| [Natural Recovery](Natural%20Recovery/NOTES.md) | own mod | 1.0.0 | yes | Overall HP regen by limb condition (+10%/h healthy, +3%/h damaged, none badly hurt), ceiling = limb %. New files only |
