...
- **Reverted (2026-09-26, at your request):** the `A.by_id` fix (addons 62, 10, Layered r11) and the grid late-redraw fix (addons 55, 10) were undone; those files are your originals again. Known leftover: dropping onto rig magazines from the backpack/pouch view logs `attempt to index field 'by_id'` (`zzz_armor_mag_pouches.script` ~22309).
- **Loot routing toggle (2026-09-26):** stash → backpack magazines and meds "vanished" because addon 10's `zzz_amp_layers.script` hard-coded `S.autoloot = true`. Every taken item was routed into a matching fitted pouch, and pouch items are hidden from the backpack list. The original Layered r11 has it off. Changed addon 10: `S.autoloot = option("layers_autoloot", false)` plus a new `zzz_amp_layers_mcm.script` (overrides Layered's) adding **MCM → Layered pouches → "Route looted items into fitted pouches"**, default **off**. LuaJIT OK; mock: no value → off, on → on.
