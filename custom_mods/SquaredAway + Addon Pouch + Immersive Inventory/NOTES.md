...
- **Fix (2026-09-26): stash → backpack items not showing until reopen.** Debug log `e49644da` showed `0 had no square` everywhere, so nothing was dropped, but `N rebuild(s) skipped` kept appearing. `zzz_amp_grid.script` skips the relayout after a window *Refresh* when the list's ids look unchanged. GAMMA's `468- Inventory Antifreeze` adds the new items a few frames **later**, so the skip fired and nothing laid them out. Changed in addons 55 (the copy that runs) and 10: when a refresh is skipped, the list is re-checked every frame for 2 s (`G.recheck`) and relaid out if items arrive late (`[GRID] late arrival in actor_bag: relayout` with debug on). If `cc.deferred` is set, the existing `G.defer_wait` path is used as well. LuaJIT compile OK; mock test: late item → 1 relayout, idle → 0. Not yet tested in-game.
